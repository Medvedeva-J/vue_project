<template>
    <div class="fav-card">
      ...
      <div class="product-info-fav">
          ...
          <div style="display: flex; flex-direction: row; margin: auto 0; vertical-align: center;">
            <p style="font-weight:bolder; padding-right:10px; ">{{product_data.price}} $</p>
            
            <button class="like-btn small-btn" @click="$store.dispatch('toggleFav', product_data.id)">
            ...
            <AmountChecker v-if="product_data.isInCart" style="margin-left:10px" :product="product_data"></AmountChecker>
          </div>
      </div>
    </div>
</template>
...
.fav-card {
  ...
  height: 20%;
  margin: 10px 0;
  padding: calc(20px + 8 * (100vw - 320px) / 880);
  margin: 20px auto;
  display: flex;
  flex-direction: row;
  border-radius: 12px;
